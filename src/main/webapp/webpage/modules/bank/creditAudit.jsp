	...
	<div class="wrapper wrapper-content">
		<div class="ibox">
    		...
    		<div class="ibox-content">
				...
				<!-- 查询条件 -->
				<div class="row">
					<form:form style="padding-left: 15px; margin-bottom: 5px;" modelAttribute="supplier_enterprise" id="searchForm" action="${ctx}/BankCreditAudit/gys-index" method="post" class="form-inline">
									...
									<tr>
										...
										<td>			  
					                 		<a href="${ctx}/BankCreditAudit/gys-info?id=${supplier_enterprise.id}" class="btn btn-info btn-xs">供应商详情</a>
							               	<c:if test="${supplier_enterprise.state == '4' and fn:contains(fns:getUser().roleNames, '银行操作员')}">
							                	<a href="javascript:void(0);" onclick="openDialogView('授信审核', '${ctx}/BankCreditAudit/form?id=${supplier_enterprise.id}','800px', '500px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 审核</a>
							               	</c:if>
							               	<c:if test="${supplier_enterprise.state == '5' and fn:contains(fns:getUser().roleNames, '银行操作员')}">
							                	<a href="javascript:void(0);" onclick="openDialog('授信配置', '${ctx}/BankCreditAudit/toUpdateAudit?id=${supplier_enterprise.id}','800px', '500px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i>授信配置</a>
							               	</c:if>
