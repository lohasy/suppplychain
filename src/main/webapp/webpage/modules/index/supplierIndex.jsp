...
<%--      				--- ${fns:getUser().roleNames}--%>
  		...
  		<div class="header_con clear">
    		...
    		<div class="menu" style="height: 100px; padding: 37px 0 0 0;">
      			<ul>
        			<li src="${ctx}/gys/gys-home?id=${fns:getUser().supplier.id}" onclick="clickTabMenu(0, this)"><a href="javascript: void(0);">首页</a></li>
        			<li src="${ctx}/yfzk/gyslist?supplierEnterpriseId.id=${fns:getUser().supplier.id}" onclick="clickTabMenu(1, this)"><a href="javascript: void(0);">融资管理</a></li>
        			<c:if test="${isYxyqgys == '1'}">
        				<li src="${ctx}/supplierChildCtrl/invitation-gys?supplierEnterpriseId.id=${fns:getUser().core.id}" onclick="clickTabMenu(2, this)"><a href="javascript: void(0);">供应商管理</a></li>
        			</c:if>
        			<c:if test="${isYxyqgys != '1'}">
        				<li style="display: none;" src="${ctx}/supplierChildCtrl/invitation-gys?supplierEnterpriseId.id=${fns:getUser().core.id}" onclick="clickTabMenu(2, this)"><a href="javascript: void(0);">供应商管理</a></li>
        			</c:if>
        			<li src="${ctx}/zhgl/gyszhgl-index" onclick="clickTabMenu(3, this)"><a href="javascript: void(0);">账户管理</a></li>
        			<li src="${ctx}/gys/gys-company-info" onclick="clickTabMenu(4, this)"><a href="javascript: void(0);">我的公司</a></li>
      			</ul>
   			</div>
  		</div>
	</div>
	...
	<div class="footer clear" style="margin: 30px 0 0 0; padding: 20px 0 20px 0;">
  		<div class="footer_con">
    		<div class="footer_nei" style="padding-top: 9px; letter-spacing: 2px;">
  				<a href="javascript: void(0);" onclick="clickTabMenu(0, this)" src="${ctx}/gys/gys-home?id=${fns:getUser().supplier.id}">首页</a> |
  				<a href="javascript: void(0);" onclick="clickTabMenu(1, this)" src="${ctx}/yfzk/gyslist?supplierEnterpriseId.id=${fns:getUser().supplier.id}">融资管理</a> |
  				<c:if test="${isYxyqgys == '1'}">
       				<a href="javascript: void(0);" onclick="clickTabMenu(2, this)" src="${ctx}/supplierChildCtrl/invitation-gys?supplierEnterpriseId.id=${fns:getUser().core.id}">供应商管理</a> |
       			</c:if>
       			<c:if test="${isYxyqgys != '1'}">
       				<a style="display: none;" href="javascript: void(0);" onclick="clickTabMenu(2, this)" src="${ctx}/supplierChildCtrl/invitation-gys?supplierEnterpriseId.id=${fns:getUser().core.id}">供应商管理</a>
       			</c:if>
  				<a href="javascript: void(0);" onclick="clickTabMenu(3, this)" src="${ctx}/zhgl/gyszhgl-index">账户管理</a> |
  				<a href="javascript: void(0);" onclick="clickTabMenu(4, this)" src="${ctx}/gys/gys-company-info">我的公司</a>
  				<br>
