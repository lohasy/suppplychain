	...
	<script>
			...
			$("#invitationGys").click(function(){
				var str="";
			  	...
			  	top.layer.confirm('您已勾选了'+ count +'位供应商，是否邀请？', {icon: 3, title:'邀请供应商'}, function(index){
			  		location.href = "${ctx}/supplierChildCtrl/batch-invitationGys?supplierEnterpriseId.id=${fns:getUser().supplier.id}&ids=" + ids;
		...
		function invitation(id){
			top.layer.confirm('是否邀请该供应商?', {icon: 3, title:'邀请供应商'}, function(index){
				location.href = "${ctx}/supplierChildCtrl/batch-invitationGys?supplierEnterpriseId.id=${fns:getUser().supplier.id}&ids=" + id;
			    top.layer.close(index);
			});
		}
		
		function refresh(){
			location.href = "${ctx}/supplierChildCtrl/invitation-gys?supplierEnterpriseId.id=${fns:getUser().supplier.id}";
		}
	</script>
</head>
<body class="gray-bg">
	<div class="rong_nav">
		<ul>
			<li><a href="${ctx}/supplierChildCtrl/invitation-gys?supplierEnterpriseId.id=${fns:getUser().supplier.id}" class="nav_a">邀请供应商</a></li>
	       	<li><a href="${ctx}/supplierChildCtrl/hxqyGys-list?supplierEnterpriseId.id=${fns:getUser().supplier.id}" class="nav_b">供应商列表</a></li>
	   	</ul>
	</div>
	<div class="wrapper wrapper-content" style="padding: 0;">
		<div class="ibox" style="margin-bottom: 0;">
			<div class="ibox-title">
				<h5><a href="${ctx}/supplierChildCtrl/invitation-gys?supplierEnterpriseId.id=${fns:getUser().supplier.id}">邀请供应商</a></h5>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;&nbsp;供应商列表</span>
    		...
    		<div class="ibox-content">
				...
				<!-- 查询条件 -->
				<div class="row">
					<form:form style="padding-left: 15px; margin-bottom: 5px;" id="searchForm" modelAttribute="supplier_child" action="${ctx}/supplierChildCtrl/invitation-gys?supplierEnterpriseId.id=${fns:getUser().supplier.id}" method="post" class="form-inline">
						...
						<div class="form-group">
							...
							<input style="margin-left: 10px;" name="supplierChildId.agencyName" value="${supplier_child.supplierChildId.agencyName}" placeholder="联系人姓名" class="input-sm form-control" />
						...
						<div class="form-group" style="display: block; margin-top: 10px;">
							...
							<select style="margin-left: 10px; padding:0 12px;" id="invitationState" name="state" val="${supplier_child.state}" class="input-sm form-control input-s-sm inline">
			        ...
			        <div class="col-sm-12" style="margin-top: 10px; margin-bottom: 10px;">
						...
						<a href="javascript:void(0);" onclick="openDialog('导入供应商', '${ctx}/gys/exportGys', '700px', '450px', 'content-frame');" class="button" id="importdata">批量导入供应商</a>
      					...
      					<a href="javascript:void(0);" onclick="openDialog('添加供应商', '${ctx}/supplierChildCtrl/hxqy-addGys', '700px', '450px', 'content-frame');" class="button2" id="newcompany">添加供应商</a>
					</div>
					<div class="col-sm-12">
							...
							<tbody>
								<c:forEach items="${page.list}" var="supplier_child">
									<tr>
										...
										<td><fmt:formatDate value="${supplier_child.exportTime}" type="both"/></td>
										<td>
											<c:if test="${supplier_child.state == 0}">
												未邀请
											</c:if>
											<c:if test="${supplier_child.state == 1}">
												已邀请
											</c:if>
											<c:if test="${supplier_child.state == 2}">
												已注册
											</c:if>
										</td>
										<td><fmt:formatDate value="${supplier_child.invitationTime}" type="both"/></td>
										<td>
											<c:if test="${supplier_child.state == 0}">
												<a href="javascript:void(0);" onclick="invitation('${supplier_child.supplierChildId.id}')" style="color: #ffffff;" class="btn btn-primary btn-xs">邀请</a>
											</c:if>
			                            	<c:if test="${supplier_child.state == 1}">
												<a href="javascript:void(0);" onclick="invitation('${supplier_child.supplierChildId.id}')" style="color: #ffffff;" class="btn btn-primary btn-xs">再次邀请</a>
											</c:if>
					                    </td>
									</tr>
								</c:forEach>
