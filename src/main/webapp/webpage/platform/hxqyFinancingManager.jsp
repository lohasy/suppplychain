...
<body class="gray-bg">
	<div class="rong_nav">
		<ul>
			<li><a href="${ctx}/yfzk/list?coreEnterpriseId.id=${fns:getUser().core.id}" class="nav_b">应付账款管理</a></li>
	       	<li><a href="${ctx}/rzglall/hxqylist?billId.coreEnterpriseId.id=${fns:getUser().core.id}" class="nav_a">融资管理</a></li>
	   	</ul>
	</div>
	<div class="wrapper wrapper-content" style="padding: 0;">
		<div class="ibox" style="margin-bottom: 0;">
			<div class="ibox-title">
				<h5><a href="${ctx}/rzglall/hxqylist?billId.coreEnterpriseId.id=${fns:getUser().core.id}">融资管理</a></h5>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;&nbsp;融资列表</span>
    		...
    		<div class="ibox-content">
				...
				<!-- 查询条件 -->
				<div class="row">
					<form:form style="padding-left: 15px; margin-bottom: 5px;" id="searchForm" modelAttribute="financing_info" action="${ctx}/rzglall/hxqylist?billId.coreEnterpriseId.id=${fns:getUser().core.id}" method="post" class="form-inline">
					...
					<div class="financing_nav clear" style="width: auto; margin-left: 15px; margin-right: 15px;">
		          		<ul id="financing-tab-area">
			            	<li id="financing-tab" class="financing_cur"><a href="${ctx}/rzglall/hxqylist?billId.coreEnterpriseId.id=${fns:getUser().core.id}">全部</a></li>
			            	<li id="financing-tab-4"><a href="${ctx}/rzglall/hxqylist?state=4&billId.coreEnterpriseId.id=${fns:getUser().core.id}">待银行审核</a></li>
			            	<li id="financing-tab-5"><a href="${ctx}/rzglall/hxqylist?state=5&billId.coreEnterpriseId.id=${fns:getUser().core.id}">审核未通过</a></li>
			            	<li id="financing-tab-6"><a href="${ctx}/rzglall/hxqylist?state=6&billId.coreEnterpriseId.id=${fns:getUser().core.id}">待银行复核</a></li>
			            	<li id="financing-tab-9"><a href="${ctx}/rzglall/hxqylist?state=9&billId.coreEnterpriseId.id=${fns:getUser().core.id}">已放款</a></li>
			            	<li id="financing-tab-10"><a href="${ctx}/rzglall/hxqylist?state=10&billId.coreEnterpriseId.id=${fns:getUser().core.id}">待银行清分</a></li>
			            	<li id="financing-tab-11"><a href="${ctx}/rzglall/hxqylist?state=11&billId.coreEnterpriseId.id=${fns:getUser().core.id}">已完成</a></li>
			          	</ul>
		        	</div>
							...
							<tbody>
								<c:forEach items="${page.list}" var="financing_info">
									<tr>
										...
										<td>${financing_info.totalAmount}</td>
					...
					<div class="financing_upload clear" style="width: auto; margin-left: 15px; margin-right: 15px;">
          				<form:form id="excel-form" action="${ctx}/rzglall/export?billId.coreEnterpriseId.id=${fns:getUser().core.id}" method="post" modelAttribute="financing_info">
