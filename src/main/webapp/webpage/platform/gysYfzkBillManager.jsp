	...
	<div class="rong_nav">
		<ul>
			<li><a href="${ctx}/yfzk/gyslist?supplierEnterpriseId.id=${fns:getUser().supplier.id}" class="nav_b">应收账款管理</a></li>
	       	...
	       	<li style="display: none;"><a href="${ctx}/yfzk/gysYfzkList?supplierParentId.id=${fns:getUser().supplier.id}" class="nav_a">应付账款管理（下级供应商）</a></li>
	...
	<div class="wrapper wrapper-content" style="padding: 0;">
		<div class="ibox" style="margin-bottom: 0;">
			<div class="ibox-title">
				<h5><a href="${ctx}/yfzk/gysYfzkList?supplierParentId.id=${fns:getUser().supplier.id}">应付账款管理</a></h5>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;&nbsp;单据列表</span>
    		...
    		<div class="ibox-content">
				...
				<!-- 查询条件 -->
				<div class="row">
					<form:form style="padding-left: 15px; margin-bottom: 5px;" id="searchForm" modelAttribute="bill_info" action="${ctx}/yfzk/gysYfzkList?supplierParentId.id=${fns:getUser().supplier.id}" method="post" class="form-inline">
					...
					<div class="financing_nav clear" style="width: auto; margin-left: 15px; margin-right: 15px;">
		          		<ul id="financing-tab-area">
			            	<li id="financing-tab" class="financing_cur"><a href="${ctx}/yfzk/gysYfzkList?supplierParentId.id=${fns:getUser().supplier.id}">全部</a></li>
			            	<li id="financing-tab-0"><a href="${ctx}/yfzk/gysYfzkList?state=0&supplierParentId.id=${fns:getUser().supplier.id}">待核心提交</a></li>
			            	<li id="financing-tab-1"><a href="${ctx}/yfzk/gysYfzkList?state=1&supplierParentId.id=${fns:getUser().supplier.id}">待供应商融资</a></li>
			            	<li id="financing-tab-11"><a href="${ctx}/yfzk/gysYfzkList?state=11&supplierParentId.id=${fns:getUser().supplier.id}">银行已放款</a></li>
			          	</ul>
		        	</div>
			        <div class="col-sm-12" style="margin-top: 15px; margin-bottom: 10px;">
			        	<a href="javascript:void(0);" style="float: left;" onclick="openDialog('上传应付账款', '${ctx}/yfzk/gysYfzkBatchUploadBillInfo', '600px', '400px', 'content-frame');" class="button" id="uploadBill">批量上传应付账款</a>
					...
					<div class="financing_upload clear" style="width: auto; margin-left: 15px; margin-right: 15px;">
						<form:form id="excel-form" action="${ctx}/yfzk/export?supplierParentId.id=${fns:getUser().supplier.id}" method="post" modelAttribute="bill_info">
