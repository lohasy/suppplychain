...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void saveRole(Role role) {
		...
		if(role.getOffice() != null && role.getOfficeList() != null) {
			role.setOfficeIdList(new ArrayList<>());
			role.getOfficeList().add(role.getOffice());
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	 ...
	 */
	@Transactional(readOnly = false)
	public void deleteUserJoin(User user) {
		if(user == null || Utils.isEmpty(user.getId())) return;
		user = userDao.get(user.getId());
		if(user == null || Utils.isEmpty(user.getId())) return;
		//删除用户与角色关系
		userDao.deleteUserRole(user);
		//删除用户与核心企业关系
		core_userDao.deleteByUser(user.getId());
		//删除用户与供应商关系
		supplier_userDao.deleteByUser(user.getId());
