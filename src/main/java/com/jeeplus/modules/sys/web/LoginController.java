...
public class LoginController extends BaseController{
	...
	public String login(HttpServletRequest request, HttpServletResponse response, Model model) throws IOException {
		...
		// 如果是手机没有登录跳转到到login，则返回JSON字符串
		 ...
		 if("supplier".equals(type)) {
			 return "modules/index/officeLogin";
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		if(us != null && !Utils.isEmpty(us.getId())) {
			...
			if(suList != null && suList.size() > 0) {
				...
				return "modules/index/supplierIndex";
			}
			...
			if(cuList != null && cuList.size() > 0) {
				return "modules/index/coreIndex";
			}
		}
	 ...
	 */
	@ResponseBody
	@RequestMapping("${adminPath}/noReadCount")
    public AjaxJson save(Monitor monitor, Model model) {
    	AjaxJson j = new AjaxJson();
    	MailBox mailBox = new MailBox();
		mailBox.setReceiver(UserUtils.getUser());
		mailBox.setReadstatus("0");//筛选未读
		j.setMsg(mailBoxService.getCount(mailBox)+"");
		return j;
    }
}
