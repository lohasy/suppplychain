...
//				currentUser.setPhoto(user.getPhoto());
	 ...
	 * @param response
	 */
	@RequestMapping(value = "updatePwdByAnscy")
	@ResponseBody
	public void updatePwdByAnscy(String oldPassword, String newPassword, HttpServletRequest request, HttpServletResponse response) {
		User user = UserUtils.getUser();
		if (user != null && !Utils.isEmpty(user.getId()) && StringUtils.isNotBlank(oldPassword) && StringUtils.isNotBlank(newPassword)){
			if (SystemService.validatePassword(oldPassword, user.getPassword())){
				systemService.updatePasswordById(user.getId(), user.getLoginName(), newPassword);
				renderString(response, "修改密码成功！");
			}else{
				renderString(response, "旧密码错误，修改密码失败！");
			}
		}else {
			renderString(response, "缺少修改密码信息，修改密码失败！");
		}
	}
